...
public class CrearLoteNotif_Controlador extends SelectorComposer<Window> {
	
	final static Logger logger = Logger.getLogger(CrearLoteNotif_Controlador.class);
	
	@Wire
	private Window winCrearLoteNotif;
	@Wire
	...
	@Listen("onCreate = #winCrearLoteNotif")
	...
	public void crearLote() {
		...
		if(!txtCodLote.getValue().isEmpty()) {
			...
			int result = dao.crearLoteNotif(listaRecibos, txtCodLote.getValue());
			if(result > 0) {
				Messagebox.show("Se han añadido " + result + " recibos al Lote Notificación " + txtCodLote.getValue());
				cancelar();
			} else {
				Messagebox.show("ERROR. Se ha producido un error al crear el Lote Notificación " + txtCodLote.getValue());
			}
		}else {
